/// first working implementation of 2-qubit kernel. for reference puroposes since kernel_n is a bit obstructed.

namespace {
	constexpr unsigned NQUBITS = 2;                        ///< number of qubits the gate acts
	...
	{
		const unsigned i = thread_id_to_offset(threadIdx.x + blockDim.x*blockIdx.x, d0, d1);

		dev_complex* matrix = reinterpret_cast<dev_complex*>(gate_matrix);
		if(i < phi_size){
			apply_gate_qid(matrix, phi + i, d0, d1);
		}
	}

	__global__ void kernel_ctrl_core(dev_complex phi[], unsigned phi_size
										 , unsigned d0, unsigned d1, unsigned ctrlmask)
	{
		const unsigned i = thread_id_to_offset(threadIdx.x + blockDim.x*blockIdx.x, d0, d1);
...
void kernel(quda::StateVector& phi, unsigned id1, unsigned id0
            , quda::Matrix const& m, size_t ctrlmask)
{
	assert(m.size()*m.size() == GATE_MATRIX_SIZE);

	auto ids = std::array<unsigned, NQUBITS>{id0, id1};
	...
	if(ctrlmask == 0){
		kernel_call(kernel_core, KernelParams1D{n_blocks, n_threads}
		            , d_phi, phi.size(), offs[0], offs[1]);
	} else {
		kernel_call(kernel_ctrl_core, KernelParams1D{n_blocks, n_threads}
		            , d_phi, phi.size(), offs[0], offs[1], ctrlmask);
	}
